...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    for (const item of checkoutItems) {
      ...
      // Use discountedPrice if available, else price from cart item (which has variant price)
      const unit =
  ...
  useEffect(() => {
    if (orderSuccess && orderSuccess.id) {
      router.replace(`/checkout/confirmation?orderId=${orderSuccess.id}`);
    }
  }, [orderSuccess, router]);
